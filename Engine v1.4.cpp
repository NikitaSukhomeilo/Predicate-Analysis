...
int main()
{
	...
	writing_filestream.close(); writing_filestream.clear();
	reading_filestream.close(); reading_filestream.clear();
	...
	while (getline(reading_filestream, file_iterator))
	{
		...
		{
			predicates[number_of_predicates++] = file_iterator;
			file_iterator.clear();
			continue;
		}
		...
		if (check_for_ending(file_iterator, RF_Adjectives_Signs) == true) // проверить на окончание возвратного глагола
		{
			file_iterator.clear();
			continue;
		}
	...
	string Buffer_For_Level2 = "Результат поиска сказуемых.txt";
	// 2 уровень
