...
string complex_verb_predicate(const string& input_word, stringstream& buffer, const string& filename)
{
	string result;
	string temp;
	...
	while (getline(reading_filestream, file_iterator))
	{
		...
		if (position != string::npos)
		{
			if (position == 0)
			{
				result += input_word;
				buffer >> temp;
				clear_substring_from_punctuation_marks(temp);
...
int main()
{
	...
	string* predicates = new string[500]; // массив для найденных в предложении сказуемых
	...
	while (getline(reading_filestream, file_iterator))
	{
		...
		while (buffer >> word) // пословное извлечение из строки
		{
			...
			{
				string CVP = complex_verb_predicate(word, buffer, "Parts_of_binding_verb_in_complex_verb_predicate.txt");
				if (CVP.size() != false)
				{
	...
	string outbound_filename = "Predicates.txt";
	wstring conv = wstring(outbound_filename.begin(), outbound_filename.end());
	LPCWSTR conv1 = conv.c_str();
	int r = CopyFile(kk, conv1, true);
	return 0;
}