	...
	while (getline(reading_filestream, temporary)) // пока извлекаем из файла строки
	{
		...
		while (buffer >> temporary) // пословное извлечение
		{
			...
			if (actual == true)
			{
				...
				{
					size_t position_found = temporary.rfind(past_time[i]) / 2;
					size_t original_length = temporary.size() / 2;
					size_t predicate_length = past_time[i].length() / 2;
					if (original_length - position_found == predicate_length)
					{
						verify_that_predicate_was_not_mentioned_yet(temporary, number_of_predicates);
						actual = false;
			...
			if(actual == true)
			{
				...
				{
					size_t position_found = temporary.rfind(imperative_mood[i]) / 2;
					size_t original_length = temporary.size() / 2;
					size_t predicate_length = imperative_mood[i].length() / 2;
					if (original_length - position_found == predicate_length)
					{
						verify_that_predicate_was_not_mentioned_yet(temporary, number_of_predicates);
						actual = false;
			...
			if (actual == true)
			{
				for (size_t i = 0; i < 2; i++)
				{
					size_t position_found = temporary.rfind(first_person[i]) / 2;
					size_t original_length = temporary.size() / 2;
					size_t predicate_length = first_person[i].length() / 2;
					if (original_length - position_found == predicate_length)
					{
						verify_that_predicate_was_not_mentioned_yet(temporary, number_of_predicates);
						actual = false;
